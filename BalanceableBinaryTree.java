...
public class BalanceableBinaryTree<K,V> extends LinkedBinaryTree<Entry<K,V>> {
	...
	public int getAux(Position<Entry<K,V>> p) {
		return ((BSTNode<Entry<K,V>>) validate(p)).getAux();
	}
	public void setAux(Position<Entry<K,V>> p, int value) {
		((BSTNode<Entry<K,V>>) validate(p)).setAux(value);
	}

	// Override node factory function to produce a BSTNode (rather than a Node)@Override
